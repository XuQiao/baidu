...
Usage: $argv[0] upload|download path_local path_remote [option(default:newcopy|overwrite)]
Usage: $argv[0] delete path_remote
Usage: $argv[0] uploadbig path_local path_remote [option(default:newcopy|overwrite)] [slice_size(default:1073741824)] [temp_dir(def:/tmp/)]
...
case 'upload':
  ...
  if(count($argv)<5){
    ...
    $res=upload_file($access_token,$argv[3],$argv[2],$argv[4]);
...
case 'uploadbig':
  ...
  case 4:
    $argv[4] = 'newcopy';
  case 5:	//设置默认值（单个文件大小->1G）
    $argv[5] = 1073741824;
    //因为需要继续下面的操作所以这里没有break
  case 6:	//设置默认值（临时文件目录->/tmp/）
    $argv[6] = '/tmp/';
    //因为需要继续下面的操作所以这里没有break
  default:	//开始上传操作
